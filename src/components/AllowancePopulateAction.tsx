...
export const AllowancePopulateAction = () => {
  ...
  return (
    <>
      <Button onClick={handleAddAllowanceDefinition}>Populate ALlowance</Button>
      <ul>
        {allowances.map((allowanceState) => (
          <li>
      ...
      <ul>
        {sharing.map((sharingState) => (
          <li>{sharingState.amountLeft}</li>
        ))}
      </ul>
    </>
  );
